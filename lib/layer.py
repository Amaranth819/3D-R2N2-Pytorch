...
class Residual_Block(nn.Module):
	def __init__(self, in_c, out_c, ks = 3, stride_ = 1, pad = 1, neg_slope = 0.1, shortcut = None):
		...
		self.path = nn.Sequential(
			nn.Conv2d(in_channels = in_c, out_channels = out_c, kernel_size = ks, stride = stride_, padding = pad, bias = False),
		...
		self.out_relu = nn.LeakyReLU(negative_slope = neg_slope, inplace = True)
...
class Recurrence_FCConv3D(nn.Module):
...
class SoftmaxLoss3D(nn.Module):
	...
	def forward(self, inputs, gt):
		...
		loss = torch.mean(
			torch.sum(-gt * adj_inputs, dim = 1, keepdim = True) + \
			torch.log(sum_exp_x)
		)
